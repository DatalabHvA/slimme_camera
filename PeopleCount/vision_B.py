...
PURPOSE
-------
This script is functionally identical to Vision_A.py, but runs on the
camera installed in Room B.

Responsibilities:
1. Capture multiple images using the Raspberry Pi camera
2. Detect people (heads) using a YOLOv8 model
3. Apply temporal filtering to stabilize detections
4. Determine whether the separation wall between Room A and Room B is open
5. Print results in a strict format for interface.py

IMPORTANT
---------
- This code has NOT been fully tested.
- All thresholds, regions, and paths are experimental.
...
import time                            #Delay between image captures
import os                              #Temporary file cleanup
...
CONF_THRESHOLD = 0.33    #Minimum confidence for valid detections
IOU_THRESHOLD = 0.5      #IoU threshold for Non-Maximum Suppression

#Temporal filtering settings
TEMPORAL_MIN_HITS = 2    #Detection must appear in ≥ N frames

#Region to mask (used to block the other classroom)
#Format: (x1, y1, x2, y2)
MASK_REGION = (0, 300, 640, 480)

#Static identifiers (used by interface.py)
CAMERA_ID = "ROOM_B"     #This script belongs to Room B

#Default wall state (overwritten by detection logic)
WALL_STATE = "CLOSED"    #"OPEN" or "CLOSED"

#Wall probe regions placed ON the wall
#sed to determine wall state based on brightness
WALL_CHECK_REGIONS = [
    (100, 320, 140, 360),
    (260, 320, 300, 360),
    (420, 320, 460, 360),
...
#Lower values imply a darker (closed) wall
...
def capture_images(picam2, num_images=3, delay=2) -> list[str]:
    ...
    - Reduces flickering and false-positive detections
...
def is_wall_closed(image_path: str) -> bool:
    ...
    analyzing brightness in predefined wall regions.

    Assumption:
    - Closed wall appears darker than open wall.
...
def get_wall_state(image_paths: list[str]) -> str:
    """
    Determines the wall state using majority voting
...
def apply_mask(image_path: str, mask_region: tuple[int, int, int, int]) -> None:
    ...
    Purpose:
    - Prevents detections in the neighboring classroom
...
def temporal_filter(detections, iou_threshold, min_hits):
    """
    Applies temporal filtering across multiple frames.

    Logic:
    - A detection must appear in multiple frames
    - Matching is based on IoU
    - Reduces false positives and unstable detections

    :param detections: List of detections per frame
    :param iou_threshold: IoU threshold for box matching
...
def count_people(image_paths: list[str]) -> tuple[int, str]:
    """
    Full detection pipeline for Room B:

    1. Detect wall state
    2. Mask irrelevant image regions
...
def main():
    ...
    IMPORTANT:
    - Called via subprocess from interface.py
    - Output format must NOT be changed
    """
